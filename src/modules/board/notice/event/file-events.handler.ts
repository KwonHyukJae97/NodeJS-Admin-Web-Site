...
export class FileEventsHandler implements IEventHandler<FileCreateEvent | TestEvent> {
  ...
  async handle(event: FileCreateEvent | TestEvent) {
    switch (event.name) {
      case FileCreateEvent.name: {
        ...
        const { boardId, files } = event as FileCreateEvent;
        // 업로드 된 파일을 해당 db에 저장하는 로직
        await this.fileService.uploadFiles(boardId, files);
        break;
      }
      case TestEvent.name: {
