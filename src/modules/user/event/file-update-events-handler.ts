...
import { FileService } from '../../account/file/file.service';
  ...
  async handle(event: FileUpdateEvent | TestEvent) {
    switch (event.name) {
      case FileUpdateEvent.name: {
        ...
        const { accountId, file } = event as FileUpdateEvent;
        // 업로드 된 파일을 해당 db에 저장 및 기존 파일 db/S3 삭제
        await this.fileService.updateFile(accountId, file);
        break;
      }
      case TestEvent.name: {
        console.log('파일 수정 최종 완료!');
        break;
      }
      default:
        break;
